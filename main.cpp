...
void display_game_over(SDL_Renderer *renderer, TTF_Font *font, int score) {
    ...
    render_text(renderer, font, "GAME OVER", (SDL_Color){255, 0, 0, 255}, SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2 - 50);

    // Display the final score
    char scoreText[64];
    sprintf(scoreText, "Final Score: %d", score);
    render_text(renderer, font, scoreText, (SDL_Color){255, 255, 255, 255}, SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2);

    // Show instructions to exit
    render_text(renderer, font, "Press ESC to Exit", (SDL_Color){255, 255, 255, 255}, SCREEN_WIDTH / 2 - 100, SCREEN_HEIGHT / 2 + 50);
...
int main(int argc, char *argv[]) {
    ...
    SDL_Texture *backgroundTexture = load_texture(renderer, "background.png"); // Load the background image
    if (!font || !eatSound || !foodTexture || !backgroundTexture) return 1;
    ...
    while (running) {
        ...
        if (snake.body[0].x < 0 || snake.body[0].x >= SCREEN_WIDTH / TILE_SIZE || 
            snake.body[0].y < 0 || snake.body[0].y >= SCREEN_HEIGHT / TILE_SIZE) running = false;
        for (int i = 1; i < snake.length; ++i)
            if (snake.body[0].x == snake.body[i].x && snake.body[0].y == snake.body[i].y) running = false;
        ...
        // Render the snake with a red body
